...
export function getServerChain() {
  if (process.env.NEXT_PUBLIC_CHAIN_ID === String(robinhoodTestnet.id)) return robinhoodTestnet;
  return process.env.NEXT_PUBLIC_CHAIN_ID === String(hardhat.id) ? hardhat : arbitrumSepolia;
}

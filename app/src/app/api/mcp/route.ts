  ...
  try {
    if (body.method === "initialize") {
      return rpc(body.id, {
        ...
        serverInfo: { name: "arbiflow-mcp", version: "0.1.0" },
        capabilities: { tools: {} }
      });
    }
...
const tools = [
  {
    name: "get_invoice",
    description: "Read an ArbiFlow invoice and its payment requirement hash.",
    inputSchema: invoiceSchema()
  },
  {
    name: "assess_invoice",
    description: "Run deterministic ArbiFlow agent policy for an invoice and optional wallet address.",
    inputSchema: {
      type: "object",
      properties: {

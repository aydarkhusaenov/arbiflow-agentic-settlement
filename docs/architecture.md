...
ArbiFlow exposes an HTTP payment interface for agents and facilitators:
  ...
  "accepts": [
    {
      ...
      "description": "Fund ArbiFlow invoice #0",
      "extra": {
        "invoiceId": "0",
...
If the invoice is already paid or finalized, `GET /api/x402/:invoiceId` returns `200` with the current invoice state instead of a new payment requirement. This lets agents treat ArbiFlow as an HTTP-native escrow resource while the contract remains the source of truth.
...
The dashboard defaults to ETH or Circle test USDC on Arbitrum Sepolia. USDC is
the intended stablecoin path for x402/EIP-3009 demos; CCTP is documented as the
production bridge for bringing native USDC onto Arbitrum before funding invoices.

...
function explorerBase(networkName, chainId) {
  ...
  if (chainId === 421614n) return EXPLORERS.arbitrumSepolia;
  if (chainId === 42161n) return EXPLORERS.arbitrumOne;
  if (chainId === 46630n) return EXPLORERS.robinhoodTestnet;
  return "";
}

function networkScriptSuffix(networkName) {
  if (networkName === "robinhoodTestnet") return "robinhood-testnet";
...
async function main() {
  ...
  if (balance === 0n && hre.network.name !== "hardhat") {
    throw new Error(`Deployer balance is 0. Fund the wallet with testnet ETH for ${hre.network.name} before deploying.`);
  }
  ...
  if (base) {
    console.log(
      `  3. (optional) Verify source:         pnpm --filter @arbiflow/contracts verify:${suffix} ${record.address}`
    );
  }
}

...
async function signValidation(escrow, invoiceId, validator, subjectAgentHash, nonce) {
  ...
  const evidenceURI = "ipfs://arbiflow/live-demo/validator-attestation";
  ...
  const schemaHash = hre.ethers.id("schema:arbiflow-live-demo-validation-v1");
...
async function main() {
  ...
  console.log(`ArbiFlow live demo on ${hre.network.name}`);
  ...
  const createReceipt = await record(
    "1. Create invoice",
    escrow.createInvoice(actor.address, ETH, amount, "ipfs://arbiflow/live-demo/autonomous-agent-loop", now + 7 * DAY, HOUR)
  );
  ...
  await record("5. Mark delivered", escrow.markDelivered(invoiceId, "ipfs://arbiflow/live-demo/delivery-proof"));
  ...
  await record(
    ...
    escrow.submitAgentFeedback(
      ...
      "demo",
      "settled",
      "ipfs://arbiflow/live-demo/feedback",

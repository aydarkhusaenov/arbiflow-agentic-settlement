...
async function seedEscrow(escrow, signers, amount) {
  ...
  await newInvoiceId(escrow, await create("ipfs://arbiflow/demo/landing-page-design"), "Created — awaiting payment");

  // 2. Unpaid invoice carrying an agent mandate + SLA (agentic accountability layer).
  const mandateId = await newInvoiceId(
    escrow,
    await create("ipfs://arbiflow/demo/ai-agent-managed-api-build"),
  ...
  await (await escrow.connect(recipient).markDelivered(releasedId, "ipfs://arbiflow/demo/delivery-audit-report")).wait();
  ...
  await (
    await escrow
      ...
      .submitAgentFeedback(
        ...
        "delivery",
        "on-time",
        "ipfs://arbiflow/demo/feedback-auditor",
  ...
  const refundedId = await newInvoiceId(
    escrow,
    await create("ipfs://arbiflow/demo/copywriting-retainer", now + 30 * DAY, HOUR),
  ...
  const settledId = await newInvoiceId(
    escrow,
    await create("ipfs://arbiflow/demo/marketing-campaign"),
  ...
  await (await escrow.connect(payer).markDisputed(settledId, "ipfs://arbiflow/demo/dispute-partial-delivery")).wait();
  await (await escrow.connect(payer).proposeSettlement(settledId, amount / 2n, "ipfs://arbiflow/demo/settlement-50-50")).wait();
  if (multiParty) {
...
async function main() {
  ...
  console.log(`Per-invoice amount: ${hre.ethers.formatEther(amount)} ETH\n`);
